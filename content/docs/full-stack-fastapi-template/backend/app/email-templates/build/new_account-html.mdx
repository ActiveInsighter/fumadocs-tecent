---
title: "new_account.html：新账户通知邮件"
description: "理解 MJML 编译后的新账户邮件、模板变量和兼容性 HTML 结构。"
---
...
管理员创建用户后，后端将项目名、登录账号、初始密码与前端入口注入模板，生成新账户通知邮件。
...
| --- | --- |
| `{{ project_name }}` | 项目显示名称。 |
| `{{ username }}` | 新账户登录邮箱或用户名。 |
| `{{ password }}` | 初始密码；生产系统应优先改为一次性设置密码流程。 |
| `{{ link }}` | 用户点击后进入前端仪表盘的地址。 |
...
```html title="new_account.html-新账户通知邮件-semantic.html" lineNumbers
<!-- 邮件内容容器：真实产物使用 table 保证邮件客户端兼容 -->
<main style="max-width: 600px; margin: auto; background: #fff">
  <h1>{{ project_name }} - New Account</h1>
  ...
  <!-- 按钮链接必须指向可信的 HTTPS 前端地址 -->
  <a href="{{ link }}" target="_blank">Go to Dashboard</a>
</main>
```
...
```html title="new_account.html" lineNumbers
