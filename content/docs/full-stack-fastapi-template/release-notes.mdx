---
title: "Release Notes：版本演进与架构变更索引"
description: "按版本记录功能、重构、修复、依赖与内部工具变化，并提供阅读方法。"
---

<Callout type="idea" title="文档定位">

发布说明不是源码教程，但它是理解项目为何形成当前架构的时间轴。遇到某段代码设计不明时，可以从对应版本的 Refactors、Fixes 和 Internal 条目追溯背景。

</Callout>

## 如何阅读

<Tabs items={['按版本', '按类别', '按 PR']}>
  <Tab>从最新版本向下看，理解当前模板最近改变了什么。</Tab>
  <Tab>只扫描 Features、Refactors、Fixes、Dependencies 等类别，追踪某类演进。</Tab>
  <Tab>打开关联 PR 查看动机、讨论、迁移风险和具体 diff。</Tab>
</Tabs>
...
| 分类 | 含义 | 学习价值 |
| --- | --- | --- |
| Features | 新增用户或开发者能力 | 观察功能从需求进入架构的路径 |
| Refactors | 行为基本不变的结构调整 | 理解当前目录与抽象来源 |
| Fixes | 缺陷修复 | 认识真实边界条件和失败模式 |
| Dependencies | 依赖升级 | 追踪框架 API 与工具链变化 |
| Internal | CI、脚本、模板维护 | 学习项目工程化与自动化 |
| Docs | 文档更新 | 找到功能使用方式和迁移说明 |
...
<Callout type="info" title="模板项目的特殊性">

模板仓库的“破坏性变化”不仅指 API，也包括生成目录、环境变量、部署配置和更新流程。升级前应阅读对应 PR，而不是只看版本号。
...
* ✏️ Fix small typo in `deployment.mdx`. PR [#1679](https://github.com/fastapi/full-stack-fastapi-template/pull/1679) by [@cassmtnr](https://github.com/cassmtnr).
...
* 📝 Update `deployment.mdx`, instructions to install GitHub Runner in non-root VMs. PR [#1412](https://github.com/fastapi/full-stack-fastapi-template/pull/1412) by [@tiangolo](https://github.com/tiangolo).
* 📝 Add MailCatcher to `development.mdx`. PR [#1387](https://github.com/fastapi/full-stack-fastapi-template/pull/1387) by [@tobiase](https://github.com/tobiase).
...
* ✏️ Fix Adminer URL in `deployment.mdx`. PR [#1194](https://github.com/tiangolo/full-stack-fastapi-template/pull/1194) by [@PhilippWu](https://github.com/PhilippWu).
...
* 📝 Fix localhost URLs in `development.mdx`. PR [#1099](https://github.com/tiangolo/full-stack-fastapi-template/pull/1099) by [@efonte](https://github.com/efonte).
...
* ✏ Fix typo in `development.mdx`. PR [#309](https://github.com/tiangolo/full-stack-fastapi-template/pull/309) by [@graue70](https://github.com/graue70).
...
* 📝 Update `README.md` and `deployment.mdx`. PR [#678](https://github.com/tiangolo/full-stack-fastapi-template/pull/678) by [@alejsdev](https://github.com/alejsdev).
