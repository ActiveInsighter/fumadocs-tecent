---
title: "deploy-production.yml：生产环境发布部署"
description: "在 Release 发布后，通过 production 自托管 Runner 构建并更新 Compose 栈。"
---

<Callout type="warn" title="工作流定位">

这是 production 环境的 Compose 部署入口。GitHub Actions 负责选择版本与注入配置，真正构建和启动容器的是带 `production` 标签的自托管 Runner。

</Callout>

## 这个工作流解决什么问题

- 把代码部署到隔离的 production 主机。
...
| 配置 | 含义 |
| --- | --- |
| `release.published` | Release 发布后部署已发布版本。 |
| `job.if` | 仅仓库 owner 不是 fastapi 时执行。 |
| `environment` | 绑定 `production` GitHub Environment。 |
...
| `DOMAIN_PRODUCTION` | 生成 DOMAIN 与 FRONTEND_HOST。 |
| `STACK_NAME_PRODUCTION` | 隔离 Compose project name。 |
...
### 2. 进入 `production` Environment，等待可能的保护审批。

  </Step>
  <Step>

### 3. 调度到带 `production` 标签的 self-hosted runner。
...
```mermaid
flowchart LR
  Event[Release 发布后部署已发布版本。] --> Guard{用户项目?}
  Guard -- 否 --> Skip[跳过]
  Guard -- 是 --> Env[GitHub Environment: production]
...
> 原始路径：`.github/workflows/deploy-production.yml`。以下仅增加学习性注释，不改变触发器、权限、表达式或命令行为。

```yaml title="deploy-production.yml" lineNumbers
...
jobs:
  deploy:
    ...
    env:
      ...
      FRONTEND_HOST: https://${{ secrets.DOMAIN_PRODUCTION }}
