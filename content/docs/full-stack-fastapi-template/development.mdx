---
title: "本地开发工作流"
description: "理解 Docker Compose Watch、热更新、测试、数据库迁移与邮件模板开发。"
---

<Callout type="idea" title="开发环境的核心目标">

本地开发不是简单地“启动两个进程”，而是让数据库、后端、前端、邮件调试和反向代理形成一个可观察、可热更新、可测试的闭环。
...
```mermaid
flowchart LR
  Edit[修改源码] --> Watch[Docker Compose Watch]
  Watch --> Backend[FastAPI 热重载]
  Watch --> Frontend[Vite HMR]
  Backend --> Tests[Pytest]
  Frontend --> E2E[Playwright]
  Backend --> Migration[Alembic 迁移]
...
1. 启动服务并理解每个容器的职责。
2. 学会进入后端容器执行命令，而不是把容器当作黑盒。
3. 修改模型后生成 Alembic 迁移，再升级数据库。
4. 用单元/集成测试验证后端，用 Playwright 验证用户流程。
...
In the guide about [deployment](deployment.mdx) you can read about Traefik, the configured proxy. That's the component in charge of transmitting traffic to the application service based on the domain.
