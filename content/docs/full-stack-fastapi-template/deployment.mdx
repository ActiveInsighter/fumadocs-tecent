---
title: "生产部署指南"
description: "从镜像构建、Traefik、HTTPS、环境变量和持续部署理解生产交付链路。"
---

<Callout type="warn" title="部署前检查">

生产部署必须替换示例密钥、确认域名 DNS 已生效，并让 HTTPS 证书、数据库备份和镜像版本具备可追踪性。
...
```mermaid
flowchart LR
  Commit[Git 提交] --> CI[GitHub Actions]
  CI --> Image[构建并推送镜像]
  Image --> Server[服务器拉取镜像]
  DNS[域名 DNS] --> Traefik[Traefik / HTTPS]
  Traefik --> App[FastAPI + 静态前端]
  App --> DB[(PostgreSQL)]
```

## 需要理解的四个边界

- **构建边界**：前端先编译为静态资源，再进入后端镜像。
- **网络边界**：公网请求先经过 Traefik，再转发到应用服务。
- **配置边界**：镜像应尽量通用，环境差异通过部署变量注入。
- **数据边界**：数据库卷和备份独立于应用镜像生命周期。
